...
| 설정 화면(AI 분석 동의·코칭 카드 표시·작성 도움)                  | 구현 완료 (Phase C-1)                         |
| 이번 주 우리(주간 리포트·패턴 관찰)                               | 구현 예정 — 자리표시자만 있음 (Phase C, 0004) |
| 추억 화면 · 대화→추억 저장 흐름                                   | 구현 예정 — 자리표시자만 있음 (Phase C)       |
...
단일 `screen` 상태(`home | chat | week | memories | settings`)와 `navigate(screen)`,
...
설정 화면에는 홈으로 돌아가는 버튼(←)을 둔다. `screen === 'settings'`일 때는 어떤 하단 탭도
`aria-current="page"`가 아니다. 상단 배너·하단 탭 크롬은 모든 화면에서 유지한다.
...
홈·이번 주 우리·추억·설정은 조건부로 렌더해 **탭을 열 때마다 새로 마운트**되고, 그때 서비스에서
